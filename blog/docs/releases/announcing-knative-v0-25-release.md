...
- **Renaming of some `net-kourier` component**
    ...
    `kubectl` **this will require a two-phase upgrade process**. In order to upgrade [net-kourier to v0.25.0](https://github.com/knative-extension/net-kourier/releases/tag/v0.25.0) using `kubectl` please follow the steps:
...
<!-- Original notes are here: https://github.com/knative-extension/eventing-kafka-broker/releases/tag/v0.25.0 -->

#### 💫 New Features & Changes

- The Trigger status contains the resolved URL of the configured dead letter sink. ([#1092](https://github.com/knative-extension/eventing-kafka-broker/pull/1092))

#### 🐞 Bug Fixes

- Fix to support subscribers that are a Kubernetes service. The service's endpoint doesn't contain a trailing slash. ([#1123](https://github.com/knative-extension/eventing-kafka-broker/pull/1123))

#### Apache Kafka Source and Channels v0.25

<!-- Original notes are here: https://github.com/knative-extension/eventing-kafka/releases/tag/v0.25.0 -->

#### 💫 New Features & Changes

- Distributed KafkaChannel Dispatcher will now re-queue KafkaChannels after failed reconciliation which will improve various failure recovery scenarios, but could consume CPU resources if reconciliation is blocked by underlying system issues (bad Kafka Secret config, etc.) ([#795](https://github.com/knative-extension/eventing-kafka/pull/795))
- Separated CRDs to extra, additional YAML manifest (similar to eventing) ([#799](https://github.com/knative-extension/eventing-kafka/pull/799))
- Enabled support for ResetOffset CRD in distributed KafkaChannel, see config/command/resetoffset for details. ([#761](https://github.com/knative-extension/eventing-kafka/pull/761))
...
- The new plugin `kn-quickstart` is now part of the home-brew plugins suite. Install the plugin using `brew install knative-extension/kn-plugins/quickstart` then use it with `kn quickstart kind` this will create a kind cluster with knative installed. Make sure to update `kn` to `v0.25` for example using `brew upgrade kn`
