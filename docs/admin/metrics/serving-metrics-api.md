---
title: "Knative Serving metrics API"
weight: 10
type: "docs"
aliases:
...
The following metrics allow cluster administrators to understand how an application responds when traffic passes through the activator.
<!--explain what the activator does-->
...
The Autoscaler component exposes metrics related to revisions of a Knative service. For example, you can monitor the number of pods the Autoscaler is allocating for a service, or the average number of requests per second during the stable window.
To read more about how autoscaler works check [here](https://github.com/knative/serving/blob/main/docs/scaling/SYSTEM.md).
<!--move these docs to one place-->
...
The following metrics are emitted by any component that implements a controller logic. The metrics show details about reconciliation operations, and which work queue behaviors on reconciliation will result in requests being added to the queue.
<!--explain this better-->
| Metric Name | Description | Type | Tags | Unit | Status |
|:-|:-|:-|:-|:-|:-|
...
Webhook metrics report useful info about operations. If a large number of operations fail this could be an issue with the submitted user resource.
...
Each Knative Serving control plane process emits a number of Go runtime [memory statistics](https://golang.org/pkg/runtime/#MemStats), known as _MemStats_.

As a baseline for monitoring purposes, you can start with a subset of the metrics:

* current allocations (`go_alloc`)
* total allocations (`go_total_alloc`)
* system memory (`go_sys`)
* mallocs (`go_mallocs`)
* frees (`go_frees`)
* garbage collection total pause time (`total_gc_pause_ns`)
* next gc target heap size (`go_next_gc`)
* number of garbage collection cycles (`num_gc`)
