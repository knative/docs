...
If you previously [created a Knative cluster](../install/_index.md), you might
already have Knative Eventing installed and running. You can check to see if the
...
If the `knative-eventing` namespace or the `in-memory-channel-controller-*` does
not exist, use the following steps to install Knative Eventing with the
in-memory channel provisioner:

1. Make sure that you have a functioning Kubernetes cluster. See the
   [Comprehensive Install guide](../install/_index.md) for more information.
...
4. Confirm that Knative Eventing is correctly installed by running the following
   command:
...
In this section you create the `event-example` namespace and then add the
`knative-eventing-injection` label to that namespace. You use namespaces to
group together and organize your Knative resources, including the Eventing
subcomponents.
...
2. Add a label to your namespace with the following command:
   ...
   This gives the `event-example` namespace the `knative-eventing-injection`
...
The [`Broker`](./broker-trigger.md#broker) ensures that every event sent by
event producers arrives at the correct event consumers. The `Broker` was created
when you labeled your namespace as ready for eventing, but it is important to
verify that your `Broker` is working correctly. In this guide, you will use the
default broker.
...
2. If `READY=False`, wait 2 minutes and re-run the command. If you continue to
...
2. To deploy the `goodbye-display` consumer to your cluster, run the following
   command:
...
3. Just like you did with the `Broker`, verify that your event consumers are
   ...
   The number of replicas in your **DESIRED** column should match the number of
...
A [Trigger](./broker-trigger.md#trigger) defines the events that you want each
of your event consumers to receive. Your `Broker` uses triggers to forward
events to the right consumers. Each trigger can specify a filter to select
relevant events based on the Cloud Event context attributes.
...
3. Verify that the triggers are working correctly by running the following
   command:
...
You have sshed into the Pod, and can now make a HTTP request. A prompt similar
...
2) To make the second request, which creates an event that has the `source`
...
3) To make the third request, which creates an event that has the `type`
...
2. Look at the logs for the `goodbye-display` event consumer by running the
   following command:
