...
the same name as its parent Deployment, which means that the container name in Knative Eventing is not the unique identifier
as it is in Knative Serving. Here is the list of containers within each Deployment resource:
...
After you create the secret, edit the KnativeEventing CR by appending the following:
...
The field `imagePullSecrets` requires a list of secrets. You can add multiple secrets to access the images as follows:
...
The field `defaultBrokerClass` indicates which class to use; if empty, the `ChannelBasedBroker` will be used.
...
The KnativeEventing CR allows you to configure system resources for the Knative system containers.
