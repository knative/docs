...
If the global key `max-scale-limit` is set, both the global max scale and per-revision max scale annotation for new revisions will be verified to not exceed it.
