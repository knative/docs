...
1. Apply the ServiceMonitors/PodMonitors to collect metrics from Knative.
    ...
    ```bash
    kubectl apply -f https://raw.githubusercontent.com/knative-extension/monitoring/main/servicemonitor.yaml
    ```

1. Grafana dashboards can be imported from the [`knative-extension` repository](https://github.com/knative-extension/monitoring/tree/main/grafana).
...
1. If you are using the Grafana Helm Chart with the Dashboard Sidecar enabled, you can load the dashboards by applying the following configmaps.
    ...
    ```bash
    kubectl apply -f https://raw.githubusercontent.com/knative-extension/monitoring/main/grafana/dashboards.yaml
    ```
...
!!! caution
    The Grafana dashboards at https://github.com/knative-extension/monitoring/tree/main/grafana don't work with metrics scraped from OpenTelemetry Collector.
