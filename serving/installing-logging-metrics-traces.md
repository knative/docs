...
If you installed one of the
[Knative install bundles](../install/README.md#installing-knative), some or all
of the observability features are installed. For example, if you install the
`release.yaml` package from the `knative/serving` repo, then an ELK stack is
installed by default and you can skip down to the
